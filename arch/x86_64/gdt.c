...
void setup_gdt(struct cpu *cpu)
{
	gdt_entries[_GDT_NULL] = (struct gdt_entry){ 0 };

	gdt_entries[_KERNEL_CS_N] = GDT_ENTRY64(
			GDT_TYPE_EXECUTABLE | GDT_TYPE_READABLE,
			0, 0xFFFFFFFF,
			GDT_DPL_KERNEL
	);
	gdt_entries[_KERNEL_DS_N] = GDT_ENTRY64(
			GDT_TYPE_WRITABLE,
			0, 0xFFFFFFFF,
			GDT_DPL_KERNEL
	);
	gdt_entries[_USER_CS_N] = GDT_ENTRY64(
			GDT_TYPE_EXECUTABLE | GDT_TYPE_READABLE,
			0, 0xFFFFFFFF,
			GDT_DPL_USER
	);
	gdt_entries[_USER_DS_N] = GDT_ENTRY64(
			GDT_TYPE_WRITABLE,
			0, 0xFFFFFFFF,
			GDT_DPL_USER
	);
	struct long_gdt_entry *entry_tss = (struct long_gdt_entry*)&gdt_entries[_GDT_TSS_N];
	*entry_tss = GDT_TSS_ENTRY64(
			((uint64_t )&cpu->cpu_state.tss),
			(sizeof(struct tss_entry64) - 1)
	);
	...
	memset(&cpu->cpu_state.tss, 0, sizeof(struct tss_entry64));
	cpu->cpu_state.tss.iopb = sizeof(struct tss_entry64);
