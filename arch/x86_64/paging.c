...
void *map_contiguous(void* pgtable, uint64_t phys_addr, void* vaddr, uint64_t size, uint32_t _2mb, uint64_t vmflags)
{
			...
			for (;pdeti < 512 && page_count < num_pages; pdeti++) {
				...
				if (_2mb) {
					pdet[pdeti] = PAGE_FRAME(phys_addr + page_count*pgsize)
						| PDET_PRESENT_FLAG | PDET_RW_FLAG | PDET_PAGE_SIZE;
					page_count++;
					continue;
				}
